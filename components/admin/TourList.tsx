...
export function TourList() {
  ...
  const [refundDialogId, setRefundDialogId] = useState<Id<'tours'> | null>(
    null
  );
  ...
  const handleCancelTour = async (tour: any) => {
    if (tour.hasPendingBookings) {
      toast.error(
        'Cannot cancel tour with PENDING bookings. Please Accept or Reject them first.'
      );
      return;
    }
    if (
      !confirm(
        'Are you sure you want to CANCEL this tour? This will stop new bookings and require you to refund existing customers.'
      )
    )
      return;
  ...
  const filteredTours = tours?.filter((tour) => {
    if (filter === 'active') {
      return !tour.isCompleted && !tour.cancelled;
    }
    if (filter === 'completed') {
      return tour.isCompleted;
    }
    if (filter === 'cancelled') {
                      ...
                      <div className="flex justify-end gap-2">
                        {isCompleted ? (
                          ...
                          >
                            <DialogTrigger asChild>
                              <Button
                                variant="outline"
                                size="sm"
                                onClick={() => setAnalyticsId(tour._id)}
                              >
                          ...
                          <>
                            {tour.cancelled ? (
                              <Dialog
                                open={refundDialogId === tour._id}
                                onOpenChange={(open) =>
                                  !open && setRefundDialogId(null)
                                }
                              >
                                <DialogTrigger asChild>
                                  <Button
                                  ...
                                  >
                                    <RefreshCw className="w-4 h-4 mr-2" /> Manage
                                    Refunds
                                  </Button>
                                </DialogTrigger>
